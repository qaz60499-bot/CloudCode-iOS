...
enum LocalVisionTextObservation {
    ...
    private static func recognizeWithHelper(
        ...
        var payload: [String: String] = [
            ...
            "localVisionAverageConfidence": response.averageConfidence.map { String($0) } ?? "0",
            "localVisionAverageConfidencePercent": response.averageConfidencePercent.map { String($0) } ?? "0",
            "localVisionMinimumConfidence": response.minimumConfidence.map { String($0) } ?? "0",
            "localVisionMaximumConfidence": response.maximumConfidence.map { String($0) } ?? "0",
