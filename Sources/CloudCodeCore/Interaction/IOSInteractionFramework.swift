...
public enum IOSInteractionFramework {
    ...
    Use the cheapest deterministic execution layer that can prove the requested transition: (1) native/system lifecycle or typed app tools, (2) structured accessibility/UI-tree elements, (3) validated App/version/iOS/device interaction experience and current-tree cache hints, (4) screenshot-driven local computer use, and only then remote Vision reasoning. Vision is a fallback, never an authority grant. A UI tree failure must fail quickly and may fall back to screenshot, but a screenshot hash/pixel change is never semantic proof.

    For a named target App, prefer a foreground-verified launch. When visual context will immediately be needed, gui.openAppObserve may combine exactly one launch with one fresh local screenshot. When accessibility exposes a unique control, prefer gui.findElement/gui.waitForElement and gui.tapElementObserve/gui.typeElementObserve instead of asking the model to infer coordinates. Element ambiguity, missing frames, stale/current-tree mismatch, or protected confirmation surfaces must fail closed.

    When several deterministic steps are already known from the current structured state, gui.runStructuredPlan may execute them locally without a provider round-trip between each step. Keep the plan bounded. Every state-changing tap/type/swipe/back step requires a local accessibility expectation before another write can run; if an expectation is late, absent, ambiguous, stale, or contradicted, stop locally and re-plan. Never put payments, authentication, passcode/biometric actions, system-permission confirmation, destructive actions, or other protected confirmations in a local multi-step plan. For a single bounded state-changing primitive whose next required step is only observation, prefer gui.tapObserve/gui.typeObserve/gui.scrollObserve/gui.swipeObserve. For an explicitly finite mechanically identical swipe repetition that needs no intermediate semantic decision, gui.swipeSequence remains valid.
