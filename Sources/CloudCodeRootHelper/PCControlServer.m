...
static BOOL CloudCodePCIsSafeBundleIdentifier(NSString *bundleID)
{
    ...
    if ([bundleID containsString:@"\n"] || [bundleID containsString:@"\r"] || [bundleID containsString:@"/"]) { return NO; }
...
static BOOL CloudCodePCIsSafeInstallRequest(NSString *path, NSString *bundleID, NSString *build)
{
    ...
    if (!CloudCodePCIsSafeBundleIdentifier(bundleID)) { return NO; }
...
static NSDictionary *CloudCodePCActionResponse(const char *executablePath, NSDictionary *request, BOOL *shutdown)
{
    ...
    if ([operation isEqualToString:@"tap"]) {
        ...
        if (!CloudCodePCIsSafeBundleIdentifier(bundleID)) {
            return @{@"ok": @NO, @"op": operation, @"error": @"invalid-bundle-identifier"};
        }
