...
The design assumes iOS suspension/termination. Agent requests persist session state and a checkpoint for each round. Active runs use a 90-minute Cloud Code background-continuation window, but this is not a promise that iOS will grant 90 minutes of uninterrupted execution: if iOS revokes background time earlier, the run is cancelled at a safe boundary and the checkpoint is preserved for automatic resume. Provider sessions wait for connectivity and retry transient timeout/connection-loss failures only before a successful 2xx streaming response has been established. Once a successful stream is established, any later transport loss becomes an interruption and is never blindly replayed; recovery proceeds from the persisted checkpoint instead. Explicit HTTP failure responses such as retryable 429/5xx remain eligible for bounded retry. Long operations should be decomposed into idempotent steps before privileged helpers or background processing expand further.
