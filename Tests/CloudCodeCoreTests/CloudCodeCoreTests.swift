...
final class CloudCodeCoreTests: XCTestCase {
    ...
    func testStructuredGUIElementToolsKeepAXSpecificToolsButPlanDoesNotRequireTreeCapability() async throws {
        ...
        let tapDescriptor = await registry.descriptor(named: "gui.tapElementObserve")
        let typeDescriptor = await registry.descriptor(named: "gui.typeElementObserve")
        let planDescriptor = await registry.descriptor(named: "gui.runStructuredPlan")
        let openObserveDescriptor = await registry.descriptor(named: "gui.openAppObserve")
        let find = try XCTUnwrap(findDescriptor)
        let wait = try XCTUnwrap(waitDescriptor)
        let tap = try XCTUnwrap(tapDescriptor)
        let type = try XCTUnwrap(typeDescriptor)
        let plan = try XCTUnwrap(planDescriptor)
        let openObserve = try XCTUnwrap(openObserveDescriptor)
    ...
    func testHarnessClassifiesPostLaunchGUIAndMessageSendRequirements() {
        ...
        XCTAssertTrue(HarnessContextManager.requiresMessageSend(in: "在微信里回复他 OK"))
        XCTAssertTrue(HarnessContextManager.requiresMessageSend(in: "回复对方消息"))
        XCTAssertFalse(HarnessContextManager.requiresMessageSend(in: "CC_NET151_ONLY_OK 只回复 OK"))
        XCTAssertFalse(HarnessContextManager.requiresPostLaunchGUIAction(in: "CC_NET151_ONLY_OK 只回复 OK"))
